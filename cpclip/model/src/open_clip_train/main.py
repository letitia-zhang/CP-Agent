...
def main(args):
    ...
    # Use enhanced CLIP if requested
    if args.use_enhanced_clip:
        model_kwargs['use_enhanced_clip'] = True
        model_kwargs['special_tokens'] = args.special_tokens
        model_kwargs['compound_dim'] = args.compound_dim


    tokenizer = get_tokenizer(args.model, context_length=args.context_length, additional_special_tokens=args.special_tokens, cache_dir=args.cache_dir)
    ...
    # Set tokenizer to enhanced model for handling string inputs and special tokens
    if hasattr(model, 'tokenizer'):
        model.tokenizer = tokenizer
        # if hasattr(model, '_add_special_tokens'):
            ...
            if optimizer is not None:
                ...
                try:
                    ...
                    valid_state = {
                        ...
                        if any(id(p) == k for p in current_param_ids.values())
                    }
                    old_opt_state["state"] = valid_state

                    optimizer.load_state_dict(old_opt_state)
                    logging.info("Optimizer state_dict loaded with adapted param_groups and cleaned state.")
