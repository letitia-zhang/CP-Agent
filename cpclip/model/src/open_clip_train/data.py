...
class JsonlDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Tokenize text here in data.py
        if self.tokenize is not None:
            text = self.tokenize([str(sample['text'])])[0]
        else:
            text = str(sample['text'])

        required_keys = ["Concentration", "Time", "compound_embedding"]
        if not all(k in sample and sample[k] is not None for k in required_keys):
            return image, text  
