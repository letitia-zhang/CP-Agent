...
from open_clip import get_input_dtype, CLIP
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                with autocast():
                    ...
                    labels = torch.arange(batch_size, device=device).long() + rank * batch_size

                    if torch.distributed.is_initialized():
                        image_features = gather_tensor(image_features)
                        text_features = gather_tensor(text_features)
                    ...
                    if logits_per_image.shape[0] != labels.shape[0]:
                        logging.warning(f"[RANK {rank}] ❗ Mismatch: logits_per_image.shape[0]={logits_per_image.shape[0]}, labels.shape[0]={labels.shape[0]}")
