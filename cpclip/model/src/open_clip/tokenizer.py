...
class SimpleTokenizer(object):
    def __init__(
        ...
        if additional_special_tokens:
            for token in additional_special_tokens:
                if token not in self.encoder:
                    self.encoder[token] = self.vocab_size
                    self.decoder[self.vocab_size] = token
                    self.vocab_size += 1
                    print(f"Registered special token: {token} → ID {self.encoder[token]}")


         # Cache for special tokens to skip BPE
        self.cache = {t: t for t in list(self.encoder.keys()) if t.startswith("<") and t.endswith(">")}

        # Build regex pattern for tokenization
        all_specials = list(special_tokens)
        if additional_special_tokens:
            all_specials.extend(additional_special_tokens)
        special_pattern = "|".join(re.escape(t) for t in all_specials)
        self.pat = re.compile(
        ...
        # special = "|".join(re.escape(t) for t in special_tokens)  # 转义特殊 token
        # self.pat = re.compile(
        #     f"(?:{special})|'s|'t|'re|'ve|'m|'ll|'d|[\p{{L}}]+|[\p{{N}}]+|[^\s\p{{L}}\p{{N}}]+",
        #     re.IGNORECASE,
        # )
        # self.vocab_size = len(self.encoder)
    ...
    def encode(self, text):
        ...
        for token in re.findall(self.pat, text):
            ...
            if token in self.encoder:  # 直接使用特殊 token 的 ID
                bpe_tokens.append(self.encoder[token])
            else:
                token_bytes = ''.join(self.byte_encoder[b] for b in token.encode('utf-8'))
                bpe_word = self.bpe(token_bytes)
                bpe_encoded = [self.encoder[bpe_token] for bpe_token in bpe_word.split(' ')]
                bpe_tokens.extend(bpe_encoded)
        print('bbbbbbb',bpe_tokens)
        return bpe_tokens
...
class HFTokenizer:
    ...
    def __init__(
        ...
        if additional_special_tokens:
            ...
            print(f"Added {num_added_tokens} special tokens to {tokenizer_name} tokenizer")
            for token in additional_special_tokens:
        ...
        self.special_token_ids = {
            ...
            for token in additional_special_tokens or []
        }
        
        # Set pad token for GPT2 if needed
        if 'gpt' in tokenizer_name.lower() and self.tokenizer.pad_token is None:
            self.tokenizer.pad_token = self.tokenizer.eos_token
            print(f"Set pad_token to eos_token for GPT tokenizer")
    ...
    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None) -> torch.Tensor:
        ...
        #     print(f"📦 Final input_ids (padded/truncated): {input_ids[i].tolist()}")
    ...
    def encode(self, text):
        """Encode text to token ids (for compatibility with SimpleTokenizer)"""
        text = self.clean_fn(text)
        tokens = self.tokenizer.encode(text, add_special_tokens=False)
        return tokens
    
    def decode(self, tokens):
        """Decode token ids to text (for compatibility with SimpleTokenizer)"""
