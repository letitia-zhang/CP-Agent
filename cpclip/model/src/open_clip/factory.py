...
from .model import CLIP, convert_weights_to_lp,\
...
def get_tokenizer(
        ...
        cache_dir: Optional[str] = None,
        tokenizer_type: str = 'hf:gpt2',  # 'auto', 'simple', 'hf:model_name'
        additional_special_tokens: Optional[List[str]] = None,
        **kwargs,
):  
    print(f"get_tokenizer called with model_name: {model_name}, context_length: {context_length}, tokenizer_type: {tokenizer_type}")
    kwargs.pop('additional_special_tokens', None)

    # Handle special tokenizer type requests
    if tokenizer_type == 'simple':
        ...
        # Support for any HuggingFace tokenizer: hf:gpt2, hf:bert-base-uncased, etc.
        hf_model_name = tokenizer_type[3:]  # Remove 'hf:' prefix
...
def create_model(
    ...
    # Check if enhanced CLIP is requested
    use_enhanced_clip = model_cfg.pop('use_enhanced_clip', False)
    
    if "multimodal_cfg" in model_cfg:
        model = CoCa(**model_cfg, cast_dtype=cast_dtype)
    elif use_enhanced_clip:
        from .model import EnhancedCLIP
        model = EnhancedCLIP(**model_cfg, cast_dtype=cast_dtype,  tokenizer=tokenizer)
    else:
